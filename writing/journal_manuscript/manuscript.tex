...
This suggests that we consider whether the scripted utterances produced by  confederates in perspective-taking tasks were actually what a speaker in that context would be expected to say \citep{KuhlenBrennan13_LanguageInDialogue}. 
...
We recruited 350 participants (170 pairs) from Amazon Mechanical Turk to play an interactive communication game using the framework described in \cite{Hawkins15_RealTimeWebExperiments}. Following our preregistered exclusion criteria\footnote{Unless }, we removed k games with 2 or more errors on non-critical items or reported confusion in an exit survey and l games containing non-native english speakers, leaving X games in our final sample. 
...
In both raw patterns of error and mouse-tracking metrics, we successfully replicated the findings of \cite{KeysarLinBarr03_LimitsOnTheoryOfMindUse}. Participants frequently attempted to move the occluded alternative when it matched the target referential expression, even though the director couldn't possibly have been referring to it. Furthermore, even on trials where they moved the correct object, matchers were more likely to hover their cursor over the occluded cell. These results support the claim that listeners neglect the speaker's perspective. 

However, when interacting with a natural 
