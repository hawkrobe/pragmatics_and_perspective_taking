...
Finally, we consider the link between our speaker and listener results. If listeners have a well-tuned pragmatic expectation about the speaker's likely level of informativity, they may rationally neglect the speaker's perspective and simply pick the object that fits the description best. To what extent does the informativity of speaker utterances account for heterogeneity in listener errors across items in the two conditions? As predicted by a perspective-negligence account, we found a strong negative correlation between informativity and error rates: listeners make fewer errors when utterances are a good fit for the target relative to the distractor ($\rho = -0.8$, bootstrapped 95\% CI $= [-0.89, -0.69]$; Fig. \ref{fig:exp1speaker}B). %  We used a mixed effects logistic regression model to estimate the effect of target and distractor fit on the probability of making a critical error in both experiments, including a random intercept for game ID. We found that participants are less likely to make errors when informativity is higher. Furthermore, a model including target fit and distractor fit in addition to item-level fixed effects is significantly better than a model including item alone, $\chi^2(2) = XXX, p < YYY$, implying that speaker informativity captures residual variance beyond the item-wise effects reported above.
...
Our primary is the length (in words) of their naturally produced referring expressions in different contexts. \ref{fig:exp2results}A

As a secondary measure (intended for use in a cognitive model), we will also annotate each utterance by the number of object features mentioned (e.g. shape, texture, color). \ref{fig:exp2results}B
