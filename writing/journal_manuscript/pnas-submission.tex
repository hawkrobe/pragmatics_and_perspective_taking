...
% TODO: Currently a bit too long (and probably not 'significant' sounding enough) 
\significancestatement{To communicate successfully, speakers must flexibly adjust the specificity of their utterances to the level demanded by the current context. In contexts where a speaker is unsure which objects their partner may be considering, it is helpful to provide additional disambiguating information: for example, saying "cassette tape" instead of the potentially ambiguous ``tape." %A listener who rationally expects such levels of specificity may, in turn, draw surprising inferences when they hear a less specific utterance than the situation demands. 
We argue that influential arguments for limits on Theory of Mind (ToM) in adults have neglected the nuanced pragmatics of the communication tasks they rest upon. A careful analysis of these pragmatics demands using a rational model of communication suggests a minimal pair of replications showing that apparent failures of visual perspective-taking are in fact successes for sophisticated social reasoning in context.}
...
\keywords{Pragmatics $|$ Theory of mind $|$ Social cognition $|$ Replication} 

\begin{abstract}
Humans are capable of accurately reasoning about other minds, but how ubiquitously do we use this ability in everyday life? Recent theories of pragmatic language use as recursive social reasoning remain in tension with evidence that adult listeners apparently fail to take their partner's perspective in communication. To reconcile these points of view, we focus attention on the problem faced by the \emph{speaker.} Through a minimal pair of replications comparing scripted vs. unscripted speakers, we show that scripted confederates used in previous studies are \emph{uncooperative}: they are less informative than what a speaker would actually produce in that situation. When we allow participants to naturally interact, we find that listener expectations are justified and errors are reduced. This ironically shows that apparent failures of theory of mind are in fact attributable to sophisticated expectations about speaker behavior--that is, to theory of mind. We formalize this argument in a computational model of pragmatic reasoning, and show that it provides a good fit with several recent findings.
\end{abstract}
