...
We consider a class of mental models representing not only a partner's visual access but also Gricean expectations of rationality and cooperativity \cite{Grice75_LogicConversation}: that speakers will avoid saying things that are confusing or unnecessarily complicated given the current context. 
...
When there is potential for confusion, however, we expect them to introduce additional modifiers (``spotted dog'') or shift to more specific terms (``Dalmatian'') to disambiguate the one they are trying to refer to \cite{BrennanClark96_ConceptualPactsConversation, GrafEtAl16_BasicLevel}. When speakers violate these expectations, listeners draw inferences about the intended meaning in order to account for the violation: for example, XXX \cite{XXX, BrehenyKatsosWilliams06_ImplicaturesOnline, StillerGoodmanFrank15_AdHocImplicature}. 

This intricate dance of expectations has recently been formalized in a probabilistic model of communication as recursive social reasoning. The resulting Rational Speech Act (RSA) framework has successfully described many puzzling pragmatic phenomena, including vagueness \cite{Lassiter}, hyperbole \cite{Kao}, irony \cite{}, metaphor, and overinformativity \cite{Judith}. In the simplest version of this framework, speakers judge the utility of different utterances proportionate to the probability of correct interpretation under an internal model of a listener. Listener, in turn, use their respective internal model of the speaker to reason backward to the most plausible intention \cite{Grice75_LogicConversation,Clark96_UsingLanguage,GoodmanFrank16_RSATiCSFrankGoodman12_PragmaticReasoningLanguageGames, GoodmanStuhlmuller13_KnowledgeImplicature}. 

Theory of mind mechanisms are therefore at the very heart of the Rational Speech Act framework, yet they have never been directly compared against egocentric or ``mindblind'' approaches under the same behavioral paradigm. By formalizing and rigorously comparing the predicted behavior of speakers and listeners using mental models of one another, we show that theory of mind is required to account for context-sensitive behavior. In addition to these novel predictions, this approach also allows us to understand the sophisticated pragmatics underlying behavior that, under more naive behavioral measures, apparently reflect failures of theory of mind. %Communication abounds with potential social expectations, and evaluating the extent to which theory of mind is deployed in communication requires considering these expectations \emph{en masse}. 
