...
Our success as a cooperative species depends upon our ability to accurately predict the behavior of other agents. The underlying mental representation we use to make such predictions --- our theory of mind \cite{PremackWoodruff78_ChimpanzeeToM} --- has been increasingly well-understood as a generative model of rational action \cite{KileyHamlinEtAl13_MentalisticCoreSocial, BakerSaxeTenenbaum09_ActionUnderstandingInversePlanning, JaraEttingerEtAl16_NaiveUtilityCalculus, BakerEtAl17_ToMBayesian}. From a young age, we can invert this model to infer another agent's likely mental states given their past behavior, and simulate their future behavior to form sophisticated expectations about how they would act in a range of scenarios \cite{SetohScottBaillargeon16_FalseBelief, ScottBaillargeon17_EarlyToM}. Yet the extent to which theory of mind is deployed in \emph{communication}, an especially ubiquitous and high-stakes site for social interaction, remains controversial. A large body of evidence has demonstrated striking failures of perspective-taking, where listeners appear to be ``mind-blind,'' neglecting the visual access of their partner when interpreting their utterances \cite{KeysarBarr___Brauner00_TakingPerspective, KeysarLinBarr03_LimitsOnTheoryOfMindUse, LinKeysarEpley10_ReflexivelyMindblind}. 

While these failures are compelling evidence against a simple model where theory of mind reasoning is limited to literal perspective-taking, we argue in this paper that the rational models speakers and listeners use to reason about their conversation partners are in fact much richer. Communication abounds with social expectations. We expect our conversation partners to follow common conventions about the meanings of words \cite{Lewis69_Convention, CentolaBaronchelli15_ConventionEmergence}, to update these conventions in light of previous interactions \cite{BrennanClark96_ConceptualPactsConversation}, and to avoid saying things that are confusing or unnecessarily complicated in the current context \cite{Grice75_LogicConversation}.




%The computational mechanisms underlying theory of mind have been increasingly well-understood as accurate predictions about another agent's future actions using a mental model of their beliefs and desires \cite{PremackWoodruff78_ChimpanzeeToM}, so too . 



%The underlying mechanisms that support our ability to communicate so flexibly and effortlessly? These expectations derive from in internal model of our partner's mind Making these predictions about our partner requires a model of their mind. rich models of someone's beliefs and desires from observing them act in the world, observe someone's behavior, we  infer the beliefs and desires  

% rational models of other minds to infer predict what our friends like (e.g. , Baker, Jara-Ettinger, etc), depends on a mental model of our conversation partner. to predict what they will say or how they will interpret our words. 

%speakers and listeners adapt their language use to their shared history and to the immediate context in rich, intricate ways \cite{Grice75_LogicConversation, Clark96_UsingLanguage}. For instance, in a context where multiple dogs may be salient to the listener, speakers introduce additional modifiers (``spotted dog'') or shift to more specific terms (``Dalmatian'') to disambiguate the one they are trying to refer to \cite{BrennanClark96_ConceptualPactsConversation, GrafEtAl16_BasicLevel}. Listeners, for their part, draw ad-hoc implicatures based on expectations that speakers will give contextually sensitive descriptions \cite{BrehenyKatsosWilliams06_ImplicaturesOnline, StillerGoodmanFrank15_AdHocImplicature} and \dots

%What underlying mechanisms support our ability to communicate so flexibly and effortlessly? 
%Children acquire this ability by at least age six \cite{WellmanCrossWatson01_ToMMetaAnalysis} and it serves as an important landmark in the developmental trajectory of intuitive theory use \cite{GopnikWellman12_ReconstructingConstructivism}.
%
...
%% Mention Westra & Carruthers as another critique focusing on pragmatics?

%% Mention neural coordination papers? e.g. Stephens et al 2010, Stolk et al, 2014?

\section*{Referring in an uncertain visual environment}

In this section, I formalize the director-matcher task inside a rational speech act (RSA) model, where speakers produce informative utterances by reasoning about a rational listener agent who updates their beliefs about the world according to the literal semantics of the language. This allows us to test pragmatic accounts of task performance. 
