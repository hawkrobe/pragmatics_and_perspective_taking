...
\caption{Mean percentage of time spent hovering over the distractor and target cells in the experimental and baseline conditions. Error bars are 95\% confidence intervals. \todo[inline]{TODO: Collapse mouse-tracking plots to a single figure}}
\label{fig:exp2hover}
\end{center}
\end{figure}
...
\todo[inline]{what else should these mixed models do? Should I just use mean rating rather than every turker's ratings? Feels like using $20\times$ the degrees of freedom is wrong, somehow.  Would be nice to see if they could account for item-wise effects explicitly somehow? Like the ``bottom block'' thing which was confusable in Exp.~1 but not Exp.~2.}
