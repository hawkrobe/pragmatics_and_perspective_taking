...
for (f in files) {
  ...
  if(!is.null(jd$answers$subject_information)) {
    newRow = data.frame(gameid = jd$answers$subject_information$gameID,
                        ...
                        understandsInstructions =  ifelse(!is.null(jd$answers$subject_information$confused), jd$answers$subject_information$confused, NA),
                        nativeEnglish =  ifelse(!is.null(jd$answers$subject_information$nativeEnglish), jd$answers$subject_information$nativeEnglish, NA),
                        ratePartner = ifelse(!is.null(jd$answers$subject_information$ratePartner), jd$answers$subject_information$ratePartner, NA),
