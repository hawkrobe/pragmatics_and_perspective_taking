...
criticalMessagesExp1 <- read.csv("../experiment1/data/processedLabelsExp1.csv") %>%
  left_join(turkerRatings, by = relevantProperties) %>%
  mutate(exp = "Exp.1")
criticalMessagesExp2 <- read.csv("../experiment2/data/processedLabelsExp2.csv") %>%
  left_join(turkerRatings, by = relevantProperties) %>%
  mutate(exp = "Exp.2")
...
# Create input for BDA

Create the informativities csv

```{r}
rbind(read_csv('../data/exp1SpeakerLabels.csv') %>% mutate(exp = 'scripted'),
      read_csv('../data/exp2SpeakerLabels.csv') %>% mutate(exp = 'unscripted')) %>%
  mutate(label = str_replace_all(label, fixed(' '), '')) %>%
  mutate(label = str_replace_all(label, "[[:punct:]]", "")) %>% 
  mutate(label = str_to_lower(label, locale = "en")) %>%
  write_csv(path = '../modeling/bdaInput/speakerData.csv')
```
...
d <- turkerRatings %>%
  filter(objectSet == 3) %>%
  group_by(label,referent) %>% 
  mutate(avg = mean(response),
            med = median(response))

ggplot(d, aes(x = response)) +
    geom_histogram(binwidth = .1) +
    geom_vline(aes(xintercept=med)) +
    facet_grid(label ~ referent) +
    theme_few() +
    theme(aspect.ratio = .25, text = element_text(size=8))

ggsave(filename = '~/Downloads/cassette_informativity_ratings.png', height = 5)
```