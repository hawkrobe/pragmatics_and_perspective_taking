...
## Model-based analyses (Appendix)

### Resource-rational simulations

```{r}
RR <- bind_rows(
  read_csv('../modeling/output/RRlistenerOutput.csv') %>% mutate(src = 'listener'),
  read_csv('../modeling/output/RRspeakerOutput.csv') %>% mutate(src ='speaker')
)

ggplot(RR, aes(x = weighting, y = prob, color = perspectiveCost, group = perspectiveCost)) +
  geom_line() +
  facet_wrap(~ src) +
  theme_few() +
  xlim(0,1) +
...
ggsave('../../pragmatics_and_perspective_taking_tex/cognitive_science_revision/figures/simulations.pdf')
```
...
```{r}
dynamics <- bind_rows(
  read_csv('../modeling/output/dynamicsOutput.csv')
)
...
```{r}
summary(glmer(hovered ~ trialType * condition + (1|gameid) + (1+trialType|objectSet), 
...
critTrials %>% 
  ...
  #mutate(error = ifelse(correct == 0, 'error', 'no error')) %>%
...
```{r}
summary(glmer(correct ~ trialNum * condition + (1 | gameid), family = 'binomial',
             data = critTrials %>%   
                    group_by(gameid) %>%
                    mutate(trialNum = row_number())))
```
...
```{r}
library(brms)
summary(brm(correct ~ trialNum * condition  + (1 + trialNum | gameid), family = 'bernoulli', 
              data = critTrials %>%   
                    group_by(gameid) %>%
                    mutate(trialNum = row_number(),
                           lastRoundIncorrect = 1-lag(correct)), control = list(adapt_delta = .95)))
```
...
```{r}
library(tidyverse)
d.supp.clicks <- read_tsv('../data/final/supplemental_exp/clickedObj/allClicks.csv')
...
```{r}
critical_only <- read_tsv('../data/final/supplemental_exp/clickedObj/allClicks.csv') %>%
  filter(occlusions =='critical') %>%
  rowwise() %>%
...
distractor <- left_join(
  read_tsv('../data/final/supplemental_exp/updateMouse/mouse.csv'),
   read_tsv('../data/final/supplemental_exp/clickedObj/allClicks.csv'),
