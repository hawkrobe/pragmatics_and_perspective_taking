...
```{r}
scripted_refs <- c('glasses', 'bottom block', 'tape', 'large measuring cup', 'brush', 'eraser', 'small candle', 'mouse')
standardized_refs <- read_csv('../data/final/experiment1/refExpressions_standardized.csv')
length(unique(c(standardized_refs$text, scripted_refs)))
unscripted_refs <- read_csv('../data/final/experiment1/refExpressions.csv')
scripted.df <- data.frame(objectSet = c(1,2,3,4,5,6,7,8), m = scripted_refs, numRawWords = c(1,2,1,3,1,1,2,1))
refs %>% 
  mutate(numRawWords = str_count(text, "\\S+")) %>%
  group_by(objectSet) %>%
