...
```{r}
subjInfo <- read_csv('../data/final/experiment2/planned_sample/turk/subjInfo.csv')
```
...
should be really straightforward but might help catch a bug for Exp. 1?

```{r}
d.mouse <- read_tsv('../data/final/experiment2/planned_sample/updateMouse/mouse.csv') %>%
  filter(!(gameid %in% badGames)) %>%
  left_join(d_annotated)

d.mouse %>%
  filter(distractorDistance != 'none') %>%
  mutate(distractorDistance = as.numeric(distractorDistance)) %>%
  group_by(gameid, trialNum) %>%
  mutate(initTime = first(localTime)) %>%
  group_by(context, hidden, gameid, trialNum, targetDistance < 300, distractorDistance < 300)  %>%
  summarize(firstTarget = first(localTime) - mean(initTime))
  # filter(distractorDistance < 300)
#  group_by()
```