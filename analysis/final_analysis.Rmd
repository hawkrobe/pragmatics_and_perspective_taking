...
ggsave('../writing/journal_manuscript/figures//hoverOverDistractor.pdf', height = 3, width = 6)
```
...
For 6 of the 8 items, unscripted speakers had significantly more words in their referring expressions... (n.s. for the other 2, but idiosyncratic)

```{r}
refs <- read_csv('../data/final/experiment1/refExpressions.csv')
scripted <- data.frame(objectSet = c(1,2,3,4,5,6,7,8), 
                       m = c('glasses', 'bottom block', 'tape', 'large measuring cup', 'brush', 'eraser', 'small candle', 'mouse'),
                       numRawWords = c(1,2,1,3,1,1,2,1))
refs %>% 
  mutate(numRawWords = str_count(text, "\\S+")) %>%
  group_by(objectSet) %>%
  tidyboot_mean(numRawWords) %>%
  left_join(scripted)
```
...
4. Just length of referring expression in words?
...
badGames <- c(incompleteIDs, location_abusers, confused)
