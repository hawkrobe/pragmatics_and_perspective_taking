...
Examine how listeners might be adapting
...
critTrials %>% 
  ...
  ggplot(aes(x= trialNum, y = n, color = lastRoundIncorrect)) +
    geom_line()
```

```{r}
library(brms)
summary(brm(correct ~ trialNum + (1 + trialNum| gameid), family = 'bernoulli', 
              data = critTrials %>%   
                    group_by(gameid) %>%
                    mutate(trialNum = row_number(),
