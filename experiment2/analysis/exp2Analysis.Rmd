...
```{r}
messageCoding1 <- read.csv("../analysis/messageCoding1.csv")
messageCoding2 <- read.csv("../analysis/messageCoding2.csv")
relevantProperties <- c("gameid", "attemptNum", "condition", "critical",
                        "objectSet", "sender", "instructionNum")
overinformativeCodes <- messageCoding1 %>% 
  left_join(messageCoding2, by = relevantProperties) %>%
  filter(!(gameid %in% bannedIDsExp2)) %>%
  mutate(contents = contents.x, time = time.x) %>%
  select(gameid, attemptNum, condition, critical, objectSet, sender, 
         instructionNum, contents, time, targetCode1, distractCode1,
         altCode1, overCode1, targetCode2, distractCode2, altCode2,
         overCode2) %>%
  mutate(avgAltCode = (altCode1 + altCode2)/2,
         avgTargetCode = (targetCode1 + targetCode2)/2,
         avgDistractCode = (distractCode1 + distractCode2)/2,
         avgOverCode = (overCode1 + overCode2)/2)

# How often are the descriptions more informative than orig?
mean(overinformativeCodes$avgOverCode)

# How often do the descriptions apply to the hidden object?
mean(overinformativeCodes$avgDistractCode)

# Does overinformativity perfectly avoid picking distractor?
cor(overinformativeCodes$avgDistractCode, overinformativeCodes$avgOverCode)

# Inter-rater reliability
kappa2(cbind(overinformativeCodes$distractCode1, overinformativeCodes$distractCode2))
kappa2(cbind(overinformativeCodes$overCode1, overinformativeCodes$overCode2))
```
...
```{r}
criticalErrors = errorDExp2 %>% 
  filter(attemptNum == 0)%>% 
  filter(criticalError == 1) 
itemWiseCodes <- overinformativeCodes %>% 
  left_join(criticalErrors, by = c("gameid", "condition", "critical", 
                                   "objectSet", "instructionNum",
                                   "attemptNum")) %>%
  select(gameid, objectSet, avgDistractCode, avgOverCode, 
         criticalError) %>%
  mutate(criticalError = ifelse(is.na(criticalError),0,1))

mmRes <- glmer(criticalError ~ avgOverCode + avgDistractCode +  (1 | objectSet),
