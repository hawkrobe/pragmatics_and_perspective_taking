...
mouseD = read.csv('mouse/exp2Mouse.csv')
errorD = read.csv('error/exp2Error.csv')
messageD = read.csv('message/exp2Message.csv')
str(mouseD)
library(tidyr)
library(dplyr)
library(ggplot2)
...
bannedIDs <- c(mistakeIDs, nonNativeEnglishIDs)
```
...
```{r}
eD <- errorD %>%
  filter(!(gameid %in% mistakeIDs)) %>%
  filter(!(gameid %in% nonNativeEnglishIDs)) %>%
  filter(critical == 1) %>%
  filter(attemptNum == 0) %>%
  ...
  group_by(condition) %>%
  summarise(atLeastOnce = length(n)/numParticipants, 
            atLeastTwice = sum(cond2)/numParticipants,
...
excludedED <- errorD %>%
  ...
  filter(critical == 1) %>%
  filter(!(objectSet %in% c(5, 6, 8))) %>% # Just 5 & 6?
  filter(attemptNum == 0) %>%
  group_by(gameid, condition) %>%
  tally() %>%
  mutate(cond2 = as.numeric(n >= 2)) %>%
